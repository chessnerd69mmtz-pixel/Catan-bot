...
function App(){
  ...
  const analysis_block=tab==="analysis"?<div className="refOverlay analysisHubOverlay"><div className="refOverlayCard analysisHubCard"><button className="refClose" onClick={closeOverlay}>×</button>
    <div className="refOverlayHead analysisHubHeader"><div><span className="eyebrow">CATAN ENGINE · GAME REVIEW</span><h2>ANALYSIS</h2><p>Open a completed game to replay every recorded move, see the engine classification for each action, and compare accuracy across every human and bot.</p></div><div className="refOverlayMetric"><span>COMPLETED GAMES</span><b>{history.length}</b></div></div>
    <section className="analysisHubControls"><div><b>GAME HISTORY ANALYZER</b><span>Only completed games are eligible for full game review.</span></div><button className="refPrimaryButton" onClick={()=>startEngineSetup()}>CUSTOM BOARD BUILDER →</button></section>
    {history.length===0?<div className="refEmpty"><h3>No completed games yet</h3><p>Finish a 10 VP 4-player match or 15 VP 1v1 match. The finished game will appear here with its result, duration and move-by-move analysis.</p><button className="refPrimaryButton" onClick={()=>{closeOverlay();startNew()}}>START NEW GAME</button></div>:<div className="analysisHistoryList">{history.slice(0,30).map(g=>{
      const moves=analysisMovesFromFrames(g.analysisFrames||[]);
      const bots=moves.filter(m=>m.isBot);
      const roleStats=analysisRoleStats(g.analysisFrames||[],g.players||[]);
      const botAccuracy=bots.length?Math.round(bots.reduce((s,m)=>s+(ANALYSIS_ACCURACY_WEIGHT[m.classification?.key]??50),0)/bots.length):null;
      return <article className="analysisHistoryCard" key={g.id}>
        <div className="analysisHistoryMain"><div><span>{fmtDate(g.date)}</span><h3>{g.result==="draw"?"DRAW":((g.winner||"MATCH")+" WON")}</h3><p>{g.gameMode==="1v1"?"1v1 · 15 VP":"4 PLAYER · 10 VP"} · {g.players?.map(p=>p.name).join(" · ")}</p></div><div className="analysisHistoryResult"><b>{g.winnerId==null?"DRAW":(g.players?.find(p=>p.id===g.winnerId)?.name||g.winner||"Winner")}</b><small>{g.targetVP} VP target</small></div></div>
        <div className="analysisHistoryStats"><div><span>RESULT</span><b>{g.result==="draw"?"DRAW":(g.winner||"—")}</b></div><div><span>TIME PLAYED</span><b>{fmtDuration(g.durationSeconds)}</b></div><div><span>GAME ACCURACY</span><b>{g.accuracy==null?"—":g.accuracy+"%"}</b></div><div><span>MOVES</span><b>{moves.length||g.analysisFrames?.length||0}</b></div><div><span>HUMAN</span><b>{roleStats.players?.[0]?.accuracy==null?"—":roleStats.players[0].accuracy+"%"}</b></div><div><span>BOTS</span><b>{botAccuracy==null?"—":botAccuracy+"%"}</b></div></div>
        <div className="analysisHistoryActions"><button className="openHistory" onClick={()=>openHistoryAnalyze(g)} disabled={!g.result||!moves.length}>ANALYZE GAME · {moves.length||g.analysisFrames?.length||0} MOVES →</button><button className="refGhostButton" onClick={()=>{setReviewGame(g);setBoard(g.board);setPlayers(g.players);setPorts(g.ports||[]);setTurn(g.winnerId||0);setWinner(g.result==="draw"?{id:null,name:"Draw",vp:0}:null);setDrawn(g.result==="draw");setLogEntries(g.logs||[]);setTab("postgame")}}>VIEW RESULT</button></div>
      </article>;
    })}</div>}
    <section className="analysisHubFooter"><div><b>Custom analysis</b><span>Choose 1v1 or 4-player, build the board and game state manually, then run the Catan Engine against that position.</span></div><button className="refGhostButton" onClick={()=>startEngineSetup()}>OPEN ANALYSIS LAB →</button></section>
  </div></div>:null;
  ...
  const replayBreakdown=ANALYSIS_CLASSIFICATIONS.map(c=>({ ...c,count:replayMoves.filter(f=>f.classification?.key===c.key).length }));
  ...
  const history_analyze_block=tab==="historyAnalyze"&&analysisReplay?.result&&replayMoves.length?<div className="refOverlay historyAnalyzeOverlay"><div className="refOverlayCard refHistoryAnalyzeCard"><button className="refClose" onClick={closeOverlay}>×</button>
    ...
    <div className="analysisReplayGrid">{replayMove?<><section className="analysisReplayBoard"><Board geo={geo} board={replayMove.after?.board||analysisReplay.board} players={replayMove.after?.players||analysisReplay.players} ports={replayMove.after?.ports||analysisReplay.ports||[]} selectedV={null} selectedE={null} analysisMode={false}/></section><aside className="analysisReplaySide">
      <section className="replayMoveCard"><div className={"classificationBadge classification-"+(replayMove.classification?.key||"good")}>{replayMove.classification?.icon||"🟢"} {replayMove.classification?.label||"GOOD"}</div><h3>{replayMove.action||"Move"}</h3><p>{replayMove.recommended?("Engine line: "+replayMove.recommended):"Recorded move from the completed match."}</p>{replayMove.engineLoss!=null&&<div className="probDelta"><span>ENGINE LOSS</span><b>{(replayMove.engineLoss*100).toFixed(1)}%</b></div>}</section>
      <section className="replayOddsCard"><div className="refPanelTitle">POSITION ODDS AFTER MOVE</div>{(replayMove.afterOdds||[]).map(o=><div className="engineOddsRow" key={o.id}><span>{o.name}</span><b>{(o.prob*100).toFixed(1)}%</b></div>)}</section>
      <section className="replayMoveList"><div className="refPanelTitle">MOVE DETAILS</div><div className="replayActionLine"><span>•</span><div><b>{replayMove.isBot?"BOT MOVE":"HUMAN MOVE"}</b><small>{replayMove.match?"Matched the engine line.":"Different from the engine top line; classified from engine loss."}</small></div></div></section>
    </aside></>:<div className="engineEmpty">No move selected.</div>}</div>
  </div></div>:null;
