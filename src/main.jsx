...
function App(){
  ...
  const analysis_block=tab==="analysis"?(
    <div className="refOverlay analysisHubOverlay"><div className="refOverlayCard analysisHubCard"><button className="refClose" onClick={closeOverlay}>×</button>
      <div className="refOverlayHead analysisHubHeader"><div><span className="eyebrow">CATAN ENGINE · GAME REVIEW</span><h2>ANALYSIS</h2><p>Completed games, move classifications, player and bot accuracy, and the custom board analyzer.</p></div><div className="refOverlayMetric"><span>COMPLETED GAMES</span><b>{history.length}</b></div></div>
      ...
      <div className="analysisHistoryList">{history.length?history.slice(0,30).map(g=><article className="analysisHistoryCard" key={g.id}>
        <div className="analysisHistoryMain"><div><span>{fmtDate(g.date)}</span><h3>{g.result==="draw"?"DRAW":(g.winner||"GAME COMPLETE")}</h3><p>{g.gameMode==="1v1"?"1v1 · 15 VP":"4 PLAYER · 10 VP"} · {(g.players||[]).map(p=>p.name).join(" · ")}</p></div><div className="analysisHistoryResult"><b>{g.result==="draw"?"DRAW":(g.winner||"WINNER")}</b><small>{g.targetVP} VP target</small></div></div>
        <div className="analysisHistoryStats"><div><span>RESULT</span><b>{g.result==="draw"?"DRAW":(g.winner||"—")}</b></div><div><span>TIME PLAYED</span><b>{fmtDuration(g.durationSeconds)}</b></div><div><span>GAME ACCURACY</span><b>{g.accuracy==null?"—":g.accuracy+"%"}</b></div><div><span>MOVES</span><b>{analysisMovesFromFrames(g.analysisFrames||[]).length}</b></div></div>
      ...
      </article>):<div className="refEmpty"><h3>No completed games yet</h3><p>Finish a game to populate the Catan Engine review archive.</p><button className="refPrimaryButton" onClick={()=>{closeOverlay();startNew()}}>START NEW GAME</button></div>}</div>
      <div className="analysisHubFooter"><div><b>Custom analysis</b><span>Choose 1v1 or 4-player, set the board, player pieces, cards, VP and side to move, then analyze the position.</span></div><button className="refGhostButton" onClick={()=>startEngineSetup()}>OPEN ANALYSIS LAB →</button></div>
    </div></div>
  ):null;
