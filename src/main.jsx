...
function App(){
  ...
  const engineDropTile=(tid,resource)=>{if(!engineBoard||!["wood","brick","sheep","wheat","ore","desert"].includes(resource))return;engineSetTile(tid,{resource,number:resource==="desert"?null:(engineBoard[tid]?.number&&engineBoard[tid].number!==7?engineBoard[tid].number:6),robber:resource==="desert"?!!engineBoard[tid]?.robber:false});setEngineSelectedTile(tid);};
  const clearEngineTile=(tid)=>{engineSetTile(tid,{resource:null,number:null,robber:false});setEngineSelectedTile(tid);};
  const startGame=()=>{
  ...
  if(screen==="engineSetup"&&engineBoard) return <div className="app engineApp">
    ...
    <main className="engineSetupPage">
      ...
      <aside className="engineSetupSide">
        ...
        <section className="engineCard sandboxBoardTools"><div className="engineCardTitle">DRAG & DROP BOARD BUILDER</div><p className="engineTiny">Drag a resource onto any hex. Drop a new resource on an occupied hex to replace it. Click a hex to edit its number or clear it.</p><div className="sandboxPalette">{[...RES,"desert"].map(r=><button key={r} draggable onDragStart={e=>{e.dataTransfer.effectAllowed="copy";e.dataTransfer.setData("catan/resource",r)}} title={`Drag ${LABEL[r]||"Desert"} to a hex`}><span>{ICON[r]}</span><b>{LABEL[r]||"Desert"}</b></button>)}</div><div className="engineToolRow"><button onClick={()=>{setEngineBoard(makeBoard(geo,mapSettings));setEnginePorts(makePorts(geo));setEngineSelectedTile(null)}}>STANDARD BOARD</button><button onClick={()=>{randomizeEngineBoard();setEngineSelectedTile(null)}}>RANDOM FILL</button></div><div className={`sandboxValidation ${engineBoardErrors.length?"hasErrors":"valid"}`}>{engineBoardErrors.length?<><b>{engineBoardErrors.length} issue{engineBoardErrors.length===1?"":"s"}</b><span>{engineBoardErrors.slice(0,4).join(" · ")}{engineBoardErrors.length>4?" …":""}</span></>:<b>BOARD VALID · READY TO START</b>}</div></section>
        <section className="engineCard"><div className="engineCardTitle">BOARD TILE</div>{engineSelectedTile==null?<p>Select a hex on the board.</p>:<><label>Resource<select value={engineBoard[engineSelectedTile]?.resource||""} onChange={e=>engineDropTile(engineSelectedTile,e.target.value)}><option value="" disabled>Choose resource…</option>{[...RES,"desert"].map(r=><option key={r} value={r}>{LABEL[r]||"Desert"}</option>)}</select></label><label>Number<input type="number" min="2" max="12" value={engineBoard[engineSelectedTile]?.number||""} disabled={!engineBoard[engineSelectedTile]?.resource||engineBoard[engineSelectedTile]?.resource==="desert"} onChange={e=>{const n=Number(e.target.value);if(n>=2&&n<=12&&n!==7)engineSetTile(engineSelectedTile,{number:n});}} placeholder="2-12, not 7"/></label><div className="engineToolRow"><button className="engineDanger" onClick={()=>clearEngineTile(engineSelectedTile)}>CLEAR HEX</button></div><label className="engineCheck"><input type="checkbox" checked={!!engineBoard[engineSelectedTile]?.robber} onChange={e=>setEngineBoard(bs=>bs.map((t,i)=>({...t,robber:i===engineSelectedTile?e.target.checked:false})))} /> Robber here</label></>}</section>
