...
function App(){
  ...
  const openHistoryAnalyze=async(g)=>{
    ...
    const game={...g,gameId};
    setAnalysisReplay(game);setAnalysisIndex(0);setAnalysisPlaying(false);setAnalysisSpeed(1);setAnalysisPauseOnError(false);setAnalysisPerspectiveId(g.players?.find(p=>!p.bot)?.id??g.players?.[0]?.id??0);setAnalysisShowAlternative(false);setAnalysisBusy(true);setTab("historyAnalyze");setScreen("playing");
