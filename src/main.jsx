...
function App(){
  ...
  {tradeHubTab==="players"&&<div className="colonistPlayerTrade"><div className="colonistPlayerSelector">{players.filter(p=>p.id!==active.id).map(p=><button key={p.id} className={Number(playerTrade.partner)===p.id?"active":""} onClick={()=>setPlayerTrade(t=>({...t,partner:p.id}))}><span className="playerTradeAvatar">●</span><b>{p.name}</b><small>{p.bot?"BOT":"PLAYER"} · {p.publicCardCount??total(p.hand)} cards</small></button>)}</div><div className="colonistTradeColumns">
    ...
    <section className="colonistTradeColumn"><div className="colonistTradeColumnTitle"><span>YOU RECEIVE</span><small>Request cards from the player</small></div><div className="colonistBundleCards">{RES.map(r=>{const n=playerTrade.getBundle[r]||0;return <div key={r} className={"colonistBundleCard receive "+(n?"chosen":"")}><span>{ICON[r]}</span><b>{LABEL[r]}</b><strong>{n}</strong><div><button disabled={!n} onClick={()=>setPlayerTrade(t=>({...t,getBundle:{...t.getBundle,[r]:n-1}})}>−</button><button onClick={()=>adjustTradeResource("get",r,1)}>+</button></div></div>})}</div></section>
