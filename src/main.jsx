...
function App(){
  ...
  const finalizeAnalysisTurn=({ps,bd,pr,bk,dk,ha,summary=""})=>{const pending=analysisTurnRef.current;if(!pending||!ps?.length||!bd?.length)return;const afterOdds=engineWinLikelihood(ps,bd,geo,pr||[],bk||emptyBank(),targetVP);const decisionsForTurn=decisionsRef.current.slice(pending.decisionStart).filter(d=>d.turn===pending.turn&&d.playerId===pending.playerId);const actions=decisionsForTurn.map(d=>d.action).filter(Boolean);const mainAction=actions.length?actions.join(" → "):summary||"Turn completed";const actedPlayer=pending.before.players.find(x=>x.id===pending.playerId)||ps.find(x=>x.id===pending.playerId);const isBotTurn=!!actedPlayer?.bot;const meBefore=pending.beforeOdds.find(x=>x.id===pending.playerId)?.prob??0.5;const meAfter=afterOdds.find(x=>x.id===pending.playerId)?.prob??meBefore;const swing=meAfter-meBefore;const losses=decisionsForTurn.map(d=>Number.isFinite(d.engineLoss)?d.engineLoss:1);const engineLoss=losses.length?Math.min(...losses):Math.max(0,-swing);const classifiedDecisions=decisionsForTurn.map(d=>({...d,swing,classification:d.classification||classifyAnalysisMove({loss:d.engineLoss??engineLoss,swing,action:d.action,gameWinning:!!d.gameWinning})}));const classification=classifyAnalysisMove({loss:engineLoss,swing,action:mainAction,gameWinning:false});const frame={turn:pending.turn,playerId:pending.playerId,playerName:pending.playerName,isBot:isBotTurn,action:mainAction,delta:swing,swing,engineLoss,classification,beforeOdds:clone(pending.beforeOdds),afterOdds:clone(afterOdds),baseline:pending.baselineLabel||"turn start",before:pending.before,after:{players:clone(ps),board:clone(bd),ports:clone(pr||[]),bank:clone(bk||emptyBank()),deckCount:Array.isArray(dk)?dk.length:0,heldAwards:clone(ha||{roadOwner:null,armyOwner:null})},decisions:clone(classifiedDecisions),moves:clone(classifiedDecisions),summary};analysisFramesRef.current=[...analysisFramesRef.current,frame];analysisTurnRef.current=null;};
