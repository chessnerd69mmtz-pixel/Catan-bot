...
const COLONIST_RULES={standard:{targetVP:10,discardIfHandOver:7,playerTrading:true,friendlyRobber:false,balancedDice:false},ranked1v1:{targetVP:15,discardIfHandOver:9,playerTrading:false,friendlyRobber:true,balancedDice:true}};
...
function createBalancedDiceController(){
  const makePairs=()=>{const a=[];for(let d1=1;d1<=6;d1++)for(let d2=1;d2<=6;d2++)a.push([d1,d2]);return a;};
  const state={pairs:makePairs(),previousSum:null,sevenStreakPlayer:null,sevenStreak:0,totalSevens:{},playerIds:[]};
  const reset=(playerIds=[])=>{state.pairs=makePairs();state.previousSum=null;state.sevenStreakPlayer=null;state.sevenStreak=0;state.totalSevens={};state.playerIds=playerIds.map(String);state.playerIds.forEach(id=>{state.totalSevens[id]=0;});};
  const roll=(playerId,playerIds=[])=>{
    const ids=playerIds.map(String);if(ids.length)state.playerIds=ids;
    if(state.pairs.length<13)state.pairs=makePairs();
    state.playerIds.forEach(id=>{state.totalSevens[id]=state.totalSevens[id]||0;});
    state.totalSevens[playerId]=state.totalSevens[playerId]||0;
    const buckets=new Map();
    for(const pair of state.pairs){const sum=pair[0]+pair[1];if(!buckets.has(sum))buckets.set(sum,[]);buckets.get(sum).push(pair);}
    const weights=new Map();let totalWeight=0;
    for(const [sum,pairs] of buckets){const streakFactor=sum===state.previousSum?0.7:1;const w=(pairs.length/state.pairs.length)*streakFactor;weights.set(sum,w);totalWeight+=w;}
    if(state.playerIds.length===2){
      const totalSevens=Object.values(state.totalSevens).reduce((a,b)=>a+b,0);
      const streakAdjustment=0.4*state.sevenStreak*(state.sevenStreakPlayer===playerId?-1:1);
      const currentSevens=state.totalSevens[playerId]||0;
      const idealPercentage=1/state.playerIds.length;
      const imbalanceAdjustment=totalSevens<state.playerIds.length?1:(1+((idealPercentage-(currentSevens/totalSevens))/idealPercentage));
      const sevenAdjustment=Math.max(0,Math.min(2,imbalanceAdjustment+streakAdjustment));
      weights.set(7,(weights.get(7)||0)*sevenAdjustment);
      totalWeight=0;for(const w of weights.values())totalWeight+=w;
    }
    let target=Math.random()*Math.max(totalWeight,1e-9),chosenSum=7;
    for(const [sum,w] of weights){if(target<=w){chosenSum=sum;break;}target-=w;}
    const list=buckets.get(chosenSum)||state.pairs.filter(p=>p[0]+p[1]===chosenSum);
    const pair=list[Math.floor(Math.random()*list.length)]||[3,4];
    const ix=state.pairs.findIndex(p=>p[0]===pair[0]&&p[1]===pair[1]);if(ix>=0)state.pairs.splice(ix,1);
    state.previousSum=chosenSum;
    if(chosenSum===7){state.totalSevens[playerId]=(state.totalSevens[playerId]||0)+1;if(state.sevenStreakPlayer===playerId)state.sevenStreak+=1;else{state.sevenStreakPlayer=playerId;state.sevenStreak=1;}}
    return pair;
  };
  return {reset,roll};
}
...
function NeedProfile(p,players,board,geo,bank,modeCfg){
  const prod=botProduction(p,board,geo),raw=empty(),need=empty(),demand=BuildingDemand(p,modeCfg);
  RES.forEach(r=>{
...
function opponentExpansionThreat(v,opp,players,board,geo,ports,bank,targetVP,modeCfg){
  ...
  if(path==null)return 0.02;
  const pathFactor=path.length===0?.92:path.length===1?.70:path.length===2?.48:path.length===3?.28:.12;
  const resourceFactor=canPay(opp.hand,COSTS.road)?.18:0;
  const pieceFactor=piecesRemaining(opp).roads>=path.length?.10:-.12;
  const openVP=openVictoryPoints(opp,players,geo,{roadOwner:null,armyOwner:null});
  const raceFactor=openVP>=targetVP-2?.12:openVP>=targetVP-4?.06:0;
  const handFactor=publicCardCount(opp)>=5?.06:0;
  return clamp(pathFactor+resourceFactor+pieceFactor+raceFactor+handFactor,0,.96);
}
function opponentTakeProbability(v,p,players,board,geo,ports,bank,targetVP,modeCfg){
  ...
  for(const opp of opponents){
    const pathThreat=opponentExpansionThreat(v,opp,players,board,geo,ports,bank,targetVP,modeCfg);
    ...
    const values=legal.map(x=>quickSpotValue(x,opp,players,board,geo,ports,bank,targetVP,modeCfg)).sort((a,b)=>b-a);
    const rank=values.findIndex(x=>Math.abs(x-target)<1e-9);
    const rankChance=rank<0?0:(rank<2?.82:rank<5?.55:rank<10?.25:.08);
    survive*=1-clamp(pathThreat*.65+rankChance*.35,0,.96);
  }
  return clamp(1-survive,0,.98);
}
...
function strategicPlanDemand(p,plan){
  ...
  if(plan?.type==='city')RES.forEach(r=>required[r]=COSTS.city[r]||0);
  if(plan?.type==='settlement'){RES.forEach(r=>required[r]=COSTS.settlement[r]||0);for(const _ of plan.path||[])RES.forEach(r=>required[r]+=COSTS.road[r]||0);}
  ...
  return {required,missing,reserved,totalMissing:RES.reduce((n,r)=>n+missing[r],0)};
}

function buildStrategicPlan(p,players,board,geo,ports,bank,targetVP,existing=null){
  let viableExisting=null;
  if(existing?.type==='settlement'&&Number.isInteger(existing.target)&&legalSettlement(existing.target,players,geo)&&piecesRemaining(p).settlements>0){
    const targetAlreadyReachable=settlementConnected(existing.target,p,geo);
    const freshPath=targetAlreadyReachable?[]:strategicRoadPathToSettlement(existing.target,p,players,geo);
    if(freshPath!==null){viableExisting={...existing,path:freshPath};}
  }else if(existing?.type==='city'&&Number.isInteger(existing.target)&&p.settlements?.includes(existing.target)&&piecesRemaining(p).cities>0){
    viableExisting={...existing};
  }
  if(viableExisting){
    const d=strategicPlanDemand(p,viableExisting);
    return {...viableExisting,demand:d.required,missing:d.missing,reserved:d.reserved,totalMissing:d.totalMissing,updatedTurn:0};
  }

  let cityPlan=null;
  if(piecesRemaining(p).cities>0&&(p.settlements||[]).length){
    for(const spot of p.settlements){
      const placement=placementScore(spot,p,players,board,geo,ports,bank,targetVP,{},null);
      const prod=spotProduction(spot,board,geo);
      const readiness=canPay(p.hand,COSTS.city)?24:buildReadiness(p,COSTS.city,board,geo)*10;
      const synergy=RES.reduce((n,r)=>n+(prod[r]||0)*(COSTS.city[r]||0),0)*12;
      const downstream=RES.reduce((n,r)=>n+(prod[r]||0)*(NeedProfile(p,players,board,geo,bank,botModeConfig(players,targetVP)).need[r]||1),0)*5;
      const race=openVictoryPoints(p,players,geo,{roadOwner:null,armyOwner:null})>=targetVP-2?10:0;
      const score=placement*.88+readiness+synergy+downstream+race;
      if(!cityPlan||score>cityPlan.score)cityPlan={type:'city',target:spot,score,createdTurn:0,reason:'Persistent city objective'};
    }
  }

  let settlementPlan=null;
  if(piecesRemaining(p).settlements>0){
    const legalCandidates=[];for(let v=0;v<geo.vertices.length;v++)if(legalSettlement(v,players,geo))legalCandidates.push(v);
    legalCandidates.sort((a,b)=>quickSpotValue(b,p,players,board,geo,ports,bank,targetVP,botModeConfig(players,targetVP))-quickSpotValue(a,p,players,board,geo,ports,bank,targetVP,botModeConfig(players,targetVP)));
    for(const v of legalCandidates.slice(0,48)){
      const spot=placementScore(v,p,players,board,geo,ports,bank,targetVP,{},null);if(!Number.isFinite(spot))continue;
      const path=strategicRoadPathToSettlement(v,p,players,geo);if(path===null)continue;
      let surviveThreat=1;for(const opp of players.filter(x=>x.id!==p.id)){surviveThreat*=1-opponentExpansionThreat(v,opp,players,board,geo,ports,bank,targetVP,botModeConfig(players,targetVP));}const threat=1-surviveThreat;
      const pathValue=Math.max(0,8-path.length)*5;
      const synergy=ComplementarityScore(v,p,players,board,geo,bank,targetVP,botModeConfig(players,targetVP))*5;
      const score=spot+pathValue+synergy+threat*7;
      if(!settlementPlan||score>settlementPlan.score)settlementPlan={type:'settlement',target:v,score,path,createdTurn:0,reason:'Persistent expansion objective'};
    }
  }

  const candidates=[cityPlan,settlementPlan].filter(Boolean).map(plan=>{
    const d=strategicPlanDemand(p,plan);
    const roadSteps=plan.type==='settlement'?(plan.path?.length||0):0;
    const estimatedTurns=Math.max(1,1+roadSteps+Math.ceil(d.totalMissing/2));
    const urgency= d.totalMissing===0?34:0;
    const race=openVictoryPoints(p,players,geo,{roadOwner:null,armyOwner:null})>=targetVP-2?18:0;
    const priority=plan.score/(1+estimatedTurns*.42)+urgency+race+(plan.type==='city'?4:0);
    return {...plan,demand:d.required,missing:d.missing,reserved:d.reserved,totalMissing:d.totalMissing,estimatedTurns,priority,updatedTurn:0};
  });
...
function strategicPlanBonus(action,plan,p,players,board,geo,ports,bank,targetVP){
  if(!plan)return 0;
  const d=strategicPlanDemand(p,plan);let bonus=0;
  if(plan.type==='city'&&action.type==='city'&&action.spot===plan.target)return 180;
  if(plan.type==='settlement'){
    if(action.type==='settlement'&&action.spot===plan.target)return 180;
    if(action.type==='road'&&(plan.path||[]).includes(action.spot))return 70+((plan.path||[]).indexOf(action.spot)===0?18:0);
  }
  if(action.type==='trade'||action.type==='playerTrade'){
    ...
    const progress=RES.reduce((n,r)=>n+Math.min(d.missing[r],get[r]),0);
    const speedFactor=plan.estimatedTurns?1+1/plan.estimatedTurns:1;
    bonus+=progress*15*speedFactor+(progress>0?8:0)+(d.totalMissing===0?18:0);
    const reservedSpend=RES.reduce((n,r)=>n+Math.max(0,(give[r]||0)-Math.max(0,(p.hand?.[r]||0)-d.reserved[r])),0);
    bonus-=reservedSpend*10;
  }
  if(action.type==='buyDev')bonus-=d.totalMissing===0?12:Math.min(18,d.totalMissing*2.8);
  if(action.type==='play'&&action.card==='Road Building'&&plan.type==='settlement')bonus+=70;
  return bonus;
}
function bestRoadBuildingPair(p,players,board,geo,ports,bank,targetVP,deadline=null,strategicPlan=null){
  ...
  const targetPath=strategicPlan?.type==='settlement'&&Number.isInteger(strategicPlan.target)?strategicRoadPathToSettlement(strategicPlan.target,p,players,geo):(strategicPlan?.path||null);
  const pathSet=new Set(targetPath||[]);
  const quick=eid=>{const e=geo.edges[eid];if(!e)return-Infinity;let score=0;for(const v of [e.a,e.b]){score+=(geo.neighbors[v]||[]).filter(n=>legalSettlement(n,players,geo)).length*2.2;score+=portAt(v,ports)?2.5:0;}if(pathSet.has(eid))score+=90-(targetPath.indexOf(eid)*5);return score;};
  const rankedLegal=legalAll.slice().sort((a,b)=>quick(b)-quick(a));
  const forcedPathEdges=targetPath||[];
  const legal=Array.from(new Set([...forcedPathEdges.filter(e=>legalAll.includes(e)),...rankedLegal.slice(0,Math.max(BOT_ROAD_CANDIDATES,32))]));
  let best=null;
  for(const a of legal){
    if(deadline!=null&&now()>=deadline-20)break;
    const v1={...p,roads:[...(p.roads||[]),a]},rp1=botSettlementOpportunityAfterRoad(a,p,players,board,geo,ports,bank,targetVP,deadline);
    let single=(rp1.best>0?rp1.best:0)*.9+(rp1.future>0?rp1.future:0)*.25+(rp1.port||0)*2.2+(pathSet.has(a)?85:0);
    if(!best||single>best.value)best={roads:[a],value:single};
    for(const b of legal){
      if(deadline!=null&&now()>=deadline-20)return best;if(b===a||!legalRoad(b,v1,players,geo))continue;
      const v2={...v1,roads:[...v1.roads,b]},rp2=botSettlementOpportunityAfterRoad(b,v1,players,board,geo,ports,bank,targetVP,deadline);
      let value=single+(rp2.best>0?rp2.best:0)*.9+(rp2.future>0?rp2.future:0)*.25+(rp2.port||0)*2.2+unlockedSettlementCount(v2,players,geo)*.35;
      if(pathSet.has(a))value+=90;if(pathSet.has(b))value+=90;if(pathSet.has(a)&&pathSet.has(b))value+=60;
      if(!best||value>best.value)best={roads:[a,b],value};
    }
  }
  return best;
}
...
function actionCandidates(p,players,board,geo,ports,bank,deck,targetVP,heldAwards,turnState={}){
  ...
  if(players.length>2){
    ...
    for(const partner of partners)for(const give of RES)for(const get of RES){if(give===get)continue;for(const ga of [1,2])for(const ra of [1,2])if((p.hand[give]||0)>=ga&&(partner.publicCardCount??total(partner.hand))>=ra)out.push({type:'playerTrade',partner:partner.id,give,get,giveAmount:ga,getAmount:ra,giveBundle:{[give]:ga},getBundle:{[get]:ra}});}
  }
  if(turnState?.botAutoplay){
    const nonTrade=out.filter(a=>!['playerTrade','trade','pass'].includes(a.type));
    const rankSettlement=(a)=>{const planBonus=plan?.type==='settlement'&&a.spot===plan.target?10000:0;return planBonus+quickSpotValue(a.spot,p,players,board,geo,ports,bank,targetVP,botModeConfig(players,targetVP));};
    const settlements=nonTrade.filter(a=>a.type==='settlement').sort((a,b)=>rankSettlement(b)-rankSettlement(a)).slice(0,BOT_SETTLEMENT_CANDIDATES);
    if(plan?.type==='settlement'&&Number.isInteger(plan.target)&&!settlements.some(a=>a.spot===plan.target)){const forced=nonTrade.find(a=>a.type==='settlement'&&a.spot===plan.target);if(forced)settlements.unshift(forced);}
    const roadRank=(a)=>{const planBonus=plan?.type==='settlement'&&(plan.path||[]).includes(a.spot)?10000:0;const e=geo.edges[a.spot];if(!e)return-Infinity;return planBonus+[e.a,e.b].reduce((n,v)=>n+(geo.neighbors[v]||[]).filter(x=>legalSettlement(x,players,geo)).length*2.2+(portAt(v,ports)?2.5:0),0);};
    const roads=nonTrade.filter(a=>a.type==='road').sort((a,b)=>roadRank(b)-roadRank(a)).slice(0,BOT_ROAD_CANDIDATES);
    for(const eid of plan?.type==='settlement'?(plan.path||[]):[]){if(!roads.some(a=>a.spot===eid)){const forced=nonTrade.find(a=>a.type==='road'&&a.spot===eid);if(forced)roads.unshift(forced);}}
    const cities=nonTrade.filter(a=>a.type==='city').sort((a,b)=>(plan?.type==='city'&&b.spot===plan.target?1:0)-(plan?.type==='city'&&a.spot===plan.target?1:0));
    const specials=nonTrade.filter(a=>!['settlement','road','city'].includes(a.type));
    const missing=r=>plan?.missing?.[r]||0;
    const bankTrades=out.filter(a=>a.type==='trade').sort((a,b)=>(missing(b.get)-missing(a.get))); 
    const playerTrades=players.length>2?out.filter(a=>a.type==='playerTrade').filter(a=>players.find(x=>x.id===a.partner)?.bot).sort((a,b)=>{const ga=a.getBundle||{},gb=b.getBundle||{};const va=RES.reduce((n,r)=>n+(ga[r]||0)*missing(r),0),vb=RES.reduce((n,r)=>n+(gb[r]||0)*missing(r),0);return vb-va;}).slice(0,BOT_TRADE_CANDIDATES):[];
    return [...settlements,...cities,...roads,...specials,...bankTrades,...playerTrades,{type:'pass'}];
  }
...
function fastTradeOpportunity(action,p,players,board,geo,ports,bank,deck,targetVP,heldAwards,cache,turnState={}){
  const give=action.type==='trade'?{[action.give]:action.rate}:normalizeBundle(action.giveBundle||{[action.give]:action.giveAmount});
  const get=action.type==='trade'?{[action.get]:1}:normalizeBundle(action.getBundle||{[action.get]:action.getAmount});
  if(!bundleCanPay(p.hand,give)||bundlesShareResource(give,get))return-Infinity;
  const after={...p,hand:add(pay(p.hand,give),get)};let value=comboCompletionValue(p.hand,after.hand,p,players)*.65;
  if(canPay(after.hand,COSTS.city))value+=24;
  if(canPay(after.hand,COSTS.settlement)){
    let best=0;for(const v of cache?.legal||[]){if(!settlementConnected(v,after,geo))continue;const score=cache.spots.get(`${p.id}:${v}`)||quickSpotValue(v,after,players,board,geo,ports,bank,targetVP,botModeConfig(players,targetVP));best=Math.max(best,score);}value+=best*1.4+10;
  }
  if(canBuyDevelopmentCard(after,deck)){const cachedCardUtility=Number.isFinite(turnState?.cardUtility)?turnState.cardUtility:0;value+=cachedCardUtility*.35;}
  const plan=turnState?.strategicPlan;
  if(plan){
    const before=plan.missing||strategicPlanDemand(p,plan).missing,afterDemand=strategicPlanDemand(after,plan);
    const progress=RES.reduce((n,r)=>n+Math.max(0,(before[r]||0)-(afterDemand.missing[r]||0)),0);
    value+=progress*15+(progress>0?8:0)+(afterDemand.totalMissing===0?30:0);
    const reserved=plan.reserved||strategicPlanDemand(p,plan).reserved;
    const overspend=RES.reduce((n,r)=>n+Math.max(0,(give[r]||0)-Math.max(0,(p.hand?.[r]||0)-reserved[r])),0);
    value-=overspend*10;
  }
  return value-Math.max(0,total(after.hand)-9)*.4;
}
...
function fastBotActionScore(action,p,players,board,geo,ports,bank,deck,targetVP,heldAwards,turnState,cache){
  const strategicBonus=strategicPlanBonus(action,turnState?.strategicPlan||null,p,players,board,geo,ports,bank,targetVP);
  const handCount=total(p.hand||empty()),pressure=handCount>=BOT_HAND_PRESSURE_THRESHOLD?Math.min(18,(handCount-(BOT_HAND_PRESSURE_THRESHOLD-1))*2.5):0;
  const racePressure=botRacePressure(p,players,geo,targetVP,heldAwards);
  const is1v1=players.length===2;
  const canImmediateCity=canPay(p.hand,COSTS.city)&&Array.isArray(p.settlements)&&p.settlements.length>0;
  const canImmediateSettlement=canPay(p.hand,COSTS.settlement)&&piecesRemaining(p).settlements>0;
  if(action.type==="play"&&action.card==="Victory Point"&&action.winReveal)return 1000;
  if(action.type==="settlement"){
    const base=placementScore(action.spot,p,players,board,geo,ports,bank,targetVP,heldAwards,cache,turnState);
    const prod=spotProduction(action.spot,board,geo);
    const existing=empty();
    (p.settlements||[]).concat(p.cities||[]).forEach(sv=>{
      const q=spotProduction(sv,board,geo);
      RES.forEach(r=>existing[r]+=q[r]);
    });
    const comboWithNetwork=pipelineComboBonus(existing,prod,is1v1)*0.45;
    const hitProb=comboHitProbability([action.spot],board,geo);
    return base+12+(portAt(action.spot,ports)?2.5:0)+pressure*.7+racePressure*.35+(canImmediateSettlement?1.5:0)+comboWithNetwork+hitProb*8+strategicBonus;
  }
  if(action.type==="city"){
    const prod=spotProduction(action.spot,board,geo);
    const pips=Object.values(prod).reduce((a,b)=>a+b,0)*36;
    const cityPipe=(prod.wheat||0)+(prod.ore||0);
    const hitProb=comboHitProbability([action.spot],board,geo);
    return 22+pips*7+(is1v1?8:2)+cityPipe*40+(p.vp>=targetVP-2?7:0)+pressure*.85+racePressure*.8+(canImmediateCity?1.0:0)+hitProb*5+strategicBonus;
  }
  if(action.type==="road"){
    const e=geo.edges[action.spot];if(!e)return-Infinity;
    let endpoint=0;
    for(const v of [e.a,e.b]){
      endpoint+=(geo.neighbors[v]||[]).filter(n=>legalSettlement(n,players,geo)).length*2.1;
      if(portAt(v,ports))endpoint+=2.4;
    }
    const before=awards(players,geo,heldAwards);
    const virtual={...p,roads:[...(p.roads||[]),action.spot]};
    const after=awards(players.map(x=>x.id===p.id?virtual:x),geo,heldAwards);
    const longest=(after.roadOwner===p.id&&before.roadOwner!==p.id)?2*BOT_LONGEST_ROAD_PRIORITY:0;
    return endpoint+longest+pressure*.25-2.5+strategicBonus;
  }
  if(action.type==="buyDev"){
    const util=turnState?.cardUtility??fastCardExpectedUtility(p,players,board,geo,ports,bank,deck,targetVP,heldAwards,turnState,cache);
    return 6+util*2+(pressure>0?pressure*1.5:0)-(canPay(p.hand,COSTS.city)||canPay(p.hand,COSTS.settlement)?3:0)+strategicBonus;
  }
  if(action.type==="trade"||action.type==="playerTrade")return fastTradeOpportunity(action,p,players,board,geo,ports,bank,deck,targetVP,heldAwards,cache,turnState)*.85+strategicBonus;
  if(action.type==="play"){
    if(action.card==="Knight"){
      const rob=chooseRobberAction(p,players,board,geo,ports,bank,targetVP,turnState?.memorySnapshots||heldAwards);
      return 5+(rob?.score||0)*.8+(p.development?.playedKnights>=2?7:0)+strategicBonus;
    }
    if(action.card==="Year of Plenty"){
      const pair=turnState?.yopPair||bestYearOfPlentyPair(p,players,board,geo,ports,bank,deck,targetVP,botModeConfig(players,targetVP),turnState);
      return 5+(pair?.value||0)*.65+strategicBonus;
    }
    if(action.card==="Monopoly"){
      const m=turnState?.monopolyTarget||bestMonopolyTarget(p,players,board,geo,ports,bank,targetVP,deck,turnState);
      return (m?.expectedGain||0)*3+(m?.value||0)*.4+strategicBonus;
    }
    if(action.card==="Road Building"){
      const rb=turnState?.roadBuildingPair||bestRoadBuildingPair(p,players,board,geo,ports,bank,targetVP,turnState?.deadline||null,turnState?.strategicPlan||null);
      return 4+(rb?.value||0)*.22+strategicBonus;
    }
  }
  return strategicBonus;
}
function scoreActions(p,players,board,geo,ports,bank,deck,targetVP,heldAwards,turnState={}){
  const deadline=turnState?.deadline??null,candidates=actionCandidates(p,players,board,geo,ports,bank,deck,targetVP,heldAwards,turnState),scored=[];
  let evaluated=0;
  const cache=turnState?.scoreCache||makeScoreCache(players,board,geo,ports,bank,targetVP);
  const planningState=turnState?.botAutoplay?{...turnState,scoreCache:cache}:turnState;
  if(turnState?.botAutoplay&&!planningState.cardUtility)planningState.cardUtility=fastCardExpectedUtility(p,players,board,geo,ports,bank,deck,targetVP,heldAwards,planningState,cache);
  for(const action of candidates){
    if(turnState?.botAutoplay&&evaluated>=BOT_MAX_ACTION_EVALUATIONS&&action.type!=="pass")break;
    if(deadline!=null){const now=typeof performance!=="undefined"&&performance.now?performance.now():Date.now();if(now>=deadline)break;}
    const score=turnState?.botAutoplay?fastBotActionScore(action,p,players,board,geo,ports,bank,deck,targetVP,heldAwards,planningState,cache):unifiedActionScore(action,p,players,board,geo,ports,bank,deck,targetVP,heldAwards,planningState);
    scored.push({...action,score});evaluated++;
  }
  ...
  settlements.forEach(a=>{
    a.portValue=TValue(a.spot,p,players,board,geo,ports,bank,targetVP,botModeConfig(players,targetVP));
    a.numberVariety=numberVarietyScore(a.spot,board,geo);
    a.portEligibleForTie=bestSettlement>0&&a.score>=bestSettlement*(1-PORT_TIE_WINDOW);
  });
  return scored.sort((a,b)=>{
    if(a.type==="settlement"&&b.type==="settlement"&&a.portEligibleForTie&&b.portEligibleForTie){
      if(Math.abs(a.score-b.score)<=Math.max(0.001,bestSettlement*PORT_TIE_WINDOW)){
        const portDelta=(b.portValue||0)-(a.portValue||0);
        if(Math.abs(portDelta)>0.001)return portDelta;
        return (b.numberVariety||0)-(a.numberVariety||0);
      }
    }
...
function bestSettlementFollowThroughAfterRoad(eid,p,players,board,geo,ports,bank,targetVP,deadline=null){
  const e=geo.edges[eid];if(!e)return null;const now=()=>typeof performance!=='undefined'&&performance.now?performance.now():Date.now();
  const virtual={...p,roads:[...(p.roads||[]),eid]},virtualPlayers=players.map(x=>x.id===p.id?virtual:x);let best=null;
  for(let v=0;v<geo.vertices.length;v++){
    if(deadline!=null&&now()>=deadline-40)break;if(!legalSettlement(v,virtualPlayers,geo))continue;
    const path=strategicRoadPathToSettlement(v,virtual,virtualPlayers,geo);if(path==null)continue;
    const score=placementScore(v,virtual,virtualPlayers,board,geo,ports,bank,targetVP,{},null)-Math.max(0,path.length-1)*1.6;
    if(!best||score>best.score)best={spot:v,score,path};
  }
  return best;
}

function bestCityFollowThrough(p,players,board,geo,ports,bank,targetVP){
  if(piecesRemaining(p).cities<=0||!canPay(p.hand,COSTS.city)||!(p.settlements||[]).length)return null;
  let best=null;
  for(const spot of p.settlements){
    const prod=spotProduction(spot,board,geo);
    const pips=Object.values(prod).reduce((a,b)=>a+b,0);
    const score=pips*12+placementScore(spot,p,players,board,geo,ports,bank,targetVP);
...
function staticStrategicValue(p,players,board,geo,ports,bank,deck,targetVP,heldAwards){
  ...
  const prod=botProduction(p,board,geo);
  const totalProd=Object.values(prod).reduce((a,b)=>a+b,0);
  const need=NeedProfile(p,players,board,geo,bank,cfg).need;
  const flexible=RES.reduce((a,r)=>a+(p.hand?.[r]||0)*need[r],0);
  const buildReady=(canPay(p.hand,COSTS.city)?3:0)+(canPay(p.hand,COSTS.settlement)&&p.settlements.length<5?3:0)+(canBuyDevelopmentCard(p,deck)?1.2:0);
  const road=connectedRoadLength(p,geo,players);
  const award=awards(players,geo,heldAwards);
  const awardVP=(award.roadOwner===p.id?2:0)+(award.armyOwner===p.id?2:0);
  const dev=Object.entries(p.development||{}).filter(([k])=>k!=='playedKnights').reduce((a,[k,n])=>a+n*(k==='Victory Point'?4.5:1.25),0);
  const portsOwned=(p.settlements||[]).concat(p.cities||[]).reduce((a,v)=>a+(portAt(v,ports)?1.8:0),0);
  return openVP*18+awardVP*12+totalProd*24+flexible*1.3+buildReady*3+road*.08+dev+portsOwned;
}
function nearPerfectBotPlan(p,players,board,geo,ports,bank,deck,targetVP,heldAwards,turnState={}){
  const scored=scoreActions(p,players,board,geo,ports,bank,deck,targetVP,heldAwards,turnState).filter(a=>a.type!=='pass'&&Number.isFinite(a.score));
  if(!scored.length)return null;
  const cfg=botModeConfig(players,targetVP);
  const roadCount=turnState.botRoadsThisTurn||0;
  const handCount=total(p.hand||empty());
  const handPressure=handCount>=BOT_HAND_PRESSURE_THRESHOLD?Math.min(18,(handCount-(BOT_HAND_PRESSURE_THRESHOLD-1))*2.5):0;
  const currentValue=0; // avoid a second full-state traversal during every bot action search
  const plannerDeadline=turnState?.deadline??null;
  const deepRoadIds=new Set(scored.filter(a=>a.type==='road').sort((a,b)=>b.score-a.score).slice(0,6).map(a=>a.spot));
  const roadAnalysis=new Map();
  const getRoadAnalysis=(eid)=>{
    if(roadAnalysis.has(eid))return roadAnalysis.get(eid);
    const rp=(plannerDeadline!=null&&((typeof performance!=="undefined"&&performance.now?performance.now():Date.now())>=plannerDeadline-70))
      ? {best:0,port:0,longest:0,future:0}
      : botSettlementOpportunityAfterRoad(eid,p,players,board,geo,ports,bank,targetVP,plannerDeadline);
    roadAnalysis.set(eid,rp);return rp;
  };
  const ranked=scored.map(action=>{
    let score=action.score;
    if(action.type==='road'){
      const rp=deepRoadIds.has(action.spot)?getRoadAnalysis(action.spot):{best:0,port:portAt(geo.edges[action.spot]?.a,ports)||portAt(geo.edges[action.spot]?.b,ports)?1:0,longest:0,future:0};
      // Longest Road is deliberately low priority. A road should normally be
      // justified by a concrete expansion/port objective, not by the award alone.
      const winsViaLongest=p.vp>=targetVP-2&&rp.longest>=2;
      const strongGoal=rp.port>0||rp.best>=11||rp.future>=13||winsViaLongest;
      // A road is a means to an end, not a default way to spend resources.
      score=score*0.10+rp.best*1.05+rp.future*0.34+rp.longest*1.0*BOT_LONGEST_ROAD_PRIORITY+rp.port*2.6;
      if(!strongGoal)score-=6.5;
      if(roadCount>=1&&!strongGoal)score-=6;
      if(roadCount>=2&&!strongGoal)score-=9;
      // If a valuable settlement/city is already affordable, road-building must clear a high bar.
      const immediateBuild=scored.some(a=>a.type==='city'||a.type==='settlement');
      if(immediateBuild&&!strongGoal)score-=9;
      // Hand-pressure mode: at 9+ cards, an otherwise sensible road gets a
      // controlled bonus because spending 2 cards now can avoid a future discard.
      if(handPressure>0&&strongGoal)score+=handPressure*1.15;
      if(handPressure>0&&!strongGoal)score-=handPressure*0.15;
    } else if(action.type==='settlement'){
      score+=4+action.score*0.28;
      if(handPressure>0)score+=handPressure*0.70;
      if(portAt(action.spot,ports))score+=1.5;
    } else if(action.type==='city'){
      score+=4+action.score*0.22;
      if(p.vp>=targetVP-2)score+=6;
      if(handPressure>0)score+=handPressure*0.90;
    } else if(action.type==='buyDev'){
      score+=cardExpectedUtility(p,players,board,geo,ports,bank,deck,targetVP,cfg)*1.8;
      if(p.vp>=targetVP-3)score+=2;
      if(canPay(p.hand,COSTS.settlement)||canPay(p.hand,COSTS.city))score-=3.5;
      // At 9+ cards, spending 3 cards on a development card is a deliberate
      // anti-discard mechanism even when the card itself is only moderately useful.
      if(handPressure>0)score+=handPressure*1.55;
    } else if(action.type==='trade'||action.type==='playerTrade'){
      const tradeValue=tradeEnablerValue(action,p,players,board,geo,ports,bank,deck,targetVP,heldAwards);
      score+=tradeValue;
      // Never make a trade simply because the raw trade evaluator is positive.
      if(tradeValue<=0)score-=3;
    } else if(action.type==='play'){
      if(action.card==='Knight'||action.card==='Monopoly')score+=3;
      if(action.card==='Year of Plenty')score+=4;
      if(action.card==='Road Building'){const rb=bestRoadBuildingPair(p,players,board,geo,ports,bank,targetVP,turnState?.deadline||null,turnState?.strategicPlan||null);score+=Math.max(0,(rb?.value||0)*0.18);}
      if(action.card==='Road Building'&&handPressure>0)score+=handPressure*0.85;
    }
    return {...action,score};
  });
  ranked.sort((a,b)=>b.score-a.score);

  // A human negotiation cannot be guaranteed to resolve inside the bot's hard
  // 5-second turn budget. Keep the normal player-trade feature available to the
  // human UI, but prevent autonomous bot turns from selecting a trade that could
  // stall waiting for a human response. Bot↔bot trades remain available.
  if(turnState?.botAutoplay){
    ranked.forEach(action=>{
      if(action.type!=="playerTrade")return;
      const partner=players.find(x=>x.id===action.partner);
      if(partner && !partner.bot)action.score=-Infinity;
    });
  }

  // Bounded opponent counterfactual: apply each candidate action to a public
  // hypothetical state, then estimate the strongest immediate response of each
  // opponent. Hidden opponent cards are never introduced into this simulation.
  const top=ranked.slice(0,6).map(action=>{
    let cfPlayers=players,cfBank=bank;
    const self=players.find(x=>x.id===p.id)||p;
    let cfSelf={...self,hand:{...self.hand},settlements:[...(self.settlements||[])],cities:[...(self.cities||[])],roads:[...(self.roads||[])]};
    if(action.type==='settlement'){cfSelf={...cfSelf,settlements:[...cfSelf.settlements,action.spot],vp:cfSelf.vp+1};}
    else if(action.type==='city'&&cfSelf.settlements.includes(action.spot)){cfSelf={...cfSelf,settlements:cfSelf.settlements.filter(v=>v!==action.spot),cities:[...cfSelf.cities,action.spot],vp:cfSelf.vp+1};}
    else if(action.type==='road'){cfSelf={...cfSelf,roads:[...cfSelf.roads,action.spot]};}
    else if(action.type==='trade'){cfSelf.hand=add(pay(cfSelf.hand,{[action.give]:action.rate}),{[action.get]:1});cfBank={...cfBank,[action.give]:(cfBank[action.give]||0)+action.rate,[action.get]:(cfBank[action.get]||0)-1};}
    else if(action.type==='playerTrade'){cfSelf.hand=add(pay(cfSelf.hand,normalizeBundle(action.giveBundle||{[action.give]:action.giveAmount})),normalizeBundle(action.getBundle||{[action.get]:action.getAmount}));}
    cfPlayers=players.map(x=>x.id===p.id?cfSelf:x);
    const opponents=cfPlayers.filter(x=>x.id!==p.id);let strongestReply=0;
    for(const opp of opponents){
      const safeOpp={...opp,hand:empty(),publicCardCount:opp.publicCardCount??total(opp.hand)};
      const oppNeed=NeedProfile(safeOpp,cfPlayers,board,geo,cfBank,cfg).need,oppProd=botProduction(safeOpp,board,geo);
      let bestSpot=0;for(let v=0;v<geo.vertices.length;v++)if(legalSettlement(v,cfPlayers,geo))bestSpot=Math.max(bestSpot,quickSpotValue(v,safeOpp,cfPlayers,board,geo,ports,cfBank,targetVP,cfg));
      let cityThreat=0;if((safeOpp.settlements||[]).length&&canPay(safeOpp.hand,COSTS.city))for(const v of safeOpp.settlements)cityThreat=Math.max(cityThreat,quickSpotValue(v,safeOpp,cfPlayers,board,geo,ports,cfBank,targetVP,cfg));
      const resourceThreat=RES.reduce((n,r)=>n+(oppProd[r]||0)*(oppNeed[r]||1),0)*2.2;
      const awardThreat=(safeOpp.development?.playedKnights||0)>=2?2:0;
      strongestReply=Math.max(strongestReply,bestSpot*.5+cityThreat*.3+resourceThreat+awardThreat);
    }
    const multiplier=action.type==='settlement'?0.18:action.type==='city'?0.12:action.type==='road'?0.10:0.06;
    action.lookahead=-(strongestReply*multiplier);action.score+=action.lookahead;return action;
  }).sort((a,b)=>b.score-a.score);

  const hasNonRoadPositive=ranked.some(a=>a.type!=='road'&&a.score>0);
  if(hasNonRoadPositive){
    const nonRoad=top.find(a=>a.type!=='road'&&a.score>0)||ranked.find(a=>a.type!=='road'&&a.score>0);
    if(nonRoad) nonRoad.score += 0.25;
    const roads=ranked.filter(a=>a.type==='road'&&a.score>0);
    roads.forEach(r=>{
      if(!deepRoadIds.has(r.spot))return;
      const rp=getRoadAnalysis(r.spot);
      const concrete=rp.port>0||rp.best>=11||rp.future>=13||(p.vp>=targetVP-2&&rp.longest>=2);
      if(!concrete) r.score-=6;
    });
    ranked.sort((a,b)=>b.score-a.score);
  }
  // At 9+ cards, never voluntarily pass while a development purchase or a
  // concrete expansion road is available. This is specifically to prevent a
  // future 7-discard without turning the bot into a road spammer.
  if(handPressure>0){
    const safety=ranked.find(a=>a.type==='buyDev'&&a.score>0)||ranked.find(a=>a.type==='road'&&a.score>0&&deepRoadIds.has(a.spot)&&(()=>{const rp=getRoadAnalysis(a.spot);return rp.port>0||rp.best>=11||rp.future>=13;})());
    if(safety) safety.score+=handPressure*0.9;
    ranked.sort((a,b)=>b.score-a.score);
  }
  const best=ranked[0]||null;
  if(!best)return null;
  best.engineTop3=ranked.filter(a=>a.score>0).slice(0,3).map(a=>({type:a.type,spot:a.spot,card:a.card,partner:a.partner,score:a.score,roads:a.roads,actionKey:decisionKeyForEngine(a)}));
  best.planValue=best.score+currentValue*.001;
  return best;
}
function roadActionPotential(eid,p,players,board,geo,targetVP,ports=[],bank=emptyBank(),plannerState=null){
  const e=geo.edges[eid];if(!e)return-Infinity;const cfg=botModeConfig(players,targetVP);let best=0;for(const v of [e.a,e.b]){const candidates=(geo.neighbors[v]||[]).filter(n=>legalSettlement(n,players,geo));best=Math.max(best,candidates.length*.8);}const before=connectedRoadLength(p,geo,players);const virtual={...p,roads:[...(p.roads||[]),eid]};const after=connectedRoadLength(virtual,geo,players);const awardBefore=awards(players,geo),awardAfter=awards(players.map(x=>x.id===p.id?virtual:x),geo);const longestGain=awardAfter.roadOwner===p.id&&awardBefore.roadOwner!==p.id?2:0;return best+(after-before)*1.2+longestGain*BOT_LONGEST_ROAD_PRIORITY-cfg.gameLengthFactor*.1;
}
...
function fastFutureSpots(v,p,players,board,geo,ports,bank,targetVP,modeCfg,cache){const key=`${p.id}:${v}`;if(cache.future.has(key))return cache.future.get(key);const future=reachableFutureSpots(v,p,players,board,geo,ports,bank,targetVP,modeCfg).slice(0,16);cache.future.set(key,future);return future;}
function fastOpponentChance(v,p,players,board,geo,ports,bank,targetVP,modeCfg,cache){
  const key=`${p.id}:${v}`;if(cache?.threats?.has(key))return cache.threats.get(key);
  let survive=1;
  for(const opp of players.filter(x=>x.id!==p.id)){
    const pathThreat=opponentExpansionThreat(v,opp,players,board,geo,ports,bank,targetVP,modeCfg);
    const rank=cache?.ranks?.get(opp.id)?.get(v);const rankChance=rank==null?0:(rank<2?.82:rank<5?.55:rank<10?.25:.08);
    survive*=1-clamp(pathThreat*.65+rankChance*.35,0,.96);
  }
  const result=clamp(1-survive,0,.98);if(cache?.threats)cache.threats.set(key,result);return result;
}
function fastEValue(v,p,players,board,geo,ports,bank,targetVP,modeCfg,cache){let totalValue=0;for(const item of fastFutureSpots(v,p,players,board,geo,ports,bank,targetVP,modeCfg,cache)){const val=cache.spots.get(`${p.id}:${item.v}`)||quickSpotValue(item.v,p,players,board,geo,ports,bank,targetVP,modeCfg);const chance=fastOpponentChance(item.v,p,players,board,geo,ports,bank,targetVP,modeCfg,cache);totalValue+=val*Math.pow(.6,item.roads)*(1-chance);}return totalValue+pathRoadBonus(v,p,players,geo);}
function fastBlockValue(v,p,players,board,geo,ports,bank,targetVP,modeCfg,cache){let best=0;for(const opp of players.filter(x=>x.id!==p.id)){const gain=cache?.spots?.get(`${opp.id}:${v}`)||quickSpotValue(v,opp,players,board,geo,ports,bank,targetVP,modeCfg);const threat=opponentExpansionThreat(v,opp,players,board,geo,ports,bank,targetVP,modeCfg);const leader=1+modeCfg.leaderMultiplier*(openVictoryPoints(opp,players,geo,{roadOwner:null,armyOwner:null})/Math.max(targetVP,1));best=Math.max(best,gain*(0.30+threat)*leader);}return best;}
...
function openingPairScore(a,b,p,players,board,geo,ports,bank=emptyBank(),targetVP=players.length===2?15:10,cache=null){
  ...
  const firstBot={...p,settlements:firstPlayers.find(x=>x.id===p.id)?.settlements||p.settlements};
  const second=openingPlacementScore(b,firstBot,firstPlayers,board,geo,ports,bank,targetVP,cache);
...
function bestOpeningPair(p,players,board,geo,ports,bank=emptyBank(),targetVP=players.length===2?15:10,budgetMs=1100){
  const start=typeof performance!=='undefined'&&performance.now?performance.now():Date.now();
  const legal=geo.vertices.map((_,i)=>i).filter(v=>legalSettlement(v,players,geo));
  const scored=legal.map(v=>({v,s:openingPlacementScore(v,p,players,board,geo,ports,bank,targetVP)})).sort((a,b)=>b.s-a.s);
  const pre=scored.slice(0,Math.min(BOT_SETUP_PAIR_CANDIDATES,scored.length)).map(x=>x.v);const cache=makeScoreCache(players,board,geo,ports,bank,targetVP);let best=null;const TIE=.02;
  for(const a of pre)for(const b of pre){
    if(a===b)continue;const now=typeof performance!=='undefined'&&performance.now?performance.now():Date.now();if(now-start>=budgetMs)return best;
    const score=openingPairScore(a,b,p,players,board,geo,ports,bank,targetVP,cache);if(!Number.isFinite(score))continue;const variety=openingNumberVarietyScore(a,b,board,geo);
    if(!best||score>best.score+Math.max(.001,Math.abs(best.score)*TIE)||(Math.abs(score-best.score)<=Math.max(.001,Math.abs(best.score)*TIE)&&variety>best.variety))best={first:a,second:b,score,variety};
  }
  return best;
}
function bestOpeningCompanion(first,p,players,board,geo,ports,bank=emptyBank(),targetVP=players.length===2?15:10,budgetMs=700){
  if(first==null)return null;const start=typeof performance!=='undefined'&&performance.now?performance.now():Date.now();
  const virtualPlayers=players.map(x=>x.id===p.id?{...x,settlements:[...(x.settlements||[]),first]}:x),virtualBot=virtualPlayers.find(x=>x.id===p.id)||p;
  const legal=geo.vertices.map((_,i)=>i).filter(v=>legalSettlement(v,virtualPlayers,geo));
  const scored=legal.map(v=>({v,s:openingPlacementScore(v,virtualBot,virtualPlayers,board,geo,ports,bank,targetVP)})).sort((a,b)=>b.s-a.s);const pre=scored.slice(0,Math.min(BOT_SETUP_PAIR_CANDIDATES,scored.length)).map(x=>x.v);
  const cache=makeScoreCache(virtualPlayers,board,geo,ports,bank,targetVP);let best=null;
  for(const b of pre){if(((typeof performance!=='undefined'&&performance.now?performance.now():Date.now())-start)>=budgetMs)break;const score=openingPairScore(first,b,virtualBot,virtualPlayers,board,geo,ports,bank,targetVP,cache);if(!Number.isFinite(score))continue;if(!best||score>best.score)best={first,second:b,score};}
  return best;
}
...
function chooseRobberAction(p,players,board,geo,ports,bank,targetVP,memorySnapshots=[]){
  ...
  const memoryPressureById=Object.fromEntries(allOpponents.map(opp=>[opp.id,memoryResourcePressure(memorySnapshots,opp.id)]));
  let best=null;
  for(let tid=0;tid<board.length;tid++){
    if(board[tid].robber)continue;
    const verts=tileVerts(geo,tid);
    if(players.length===2&&allOpponents.some(opp=>friendlyRobberProtected(opp,players,geo,{roadOwner:null,armyOwner:null})&&verts.some(v=>opp.settlements.includes(v)||opp.cities.includes(v))))continue;
    const pip=PROB[board[tid].number]||0;if(!pip)continue;
    let totalBlockedPips=0,totalDenial=0,totalHand=0,historical=0,leaderPressure=0,selfPips=0,bestVictim=null,bestVictimScore=-1;
    for(const v of verts){if(p.settlements.includes(v))selfPips+=pip;if(p.cities.includes(v))selfPips+=pip*2;}
    for(const opp of allOpponents){
      ...
      if(pips<=0)continue;
      totalBlockedPips+=pips;totalHand+=publicCardCount(opp);
      const need=NeedProfile(opp,players,board,geo,bank,cfg).need;
      const prod=spotProductionVertexTileLess(tid,board);
      const openVP=openVictoryPoints(opp,players,geo,{roadOwner:null,armyOwner:null});
      totalDenial+=RES.reduce((a,r)=>a+prod[r]*need[r],0)*pips/Math.max(pip,1);
      const pressure=memoryPressureById[opp.id]||empty();historical+=RES.reduce((a,r)=>a+prod[r]*Math.max(0,pressure[r]-1.5),0)*1.7;
      leaderPressure+=(openVP/Math.max(targetVP,1))*cfg.leaderMultiplier;
      const resourcePlanThreat=RES.reduce((a,r)=>a+(prod[r]||0)*(need[r]||1)*80,0);
      const steal=publicCardCount(opp)>0?1.8:0.15;
      const victimScore=resourcePlanThreat+pips*steal+openVP*2.4+(players.length>2?openVP*1.6:0);
      if(victimScore>bestVictimScore){bestVictim=opp;bestVictimScore=victimScore;}
    }
    if(totalBlockedPips<=0)continue;
    const score=totalBlockedPips*8 + totalDenial*95 + leaderPressure*14 + totalHand*.02 + historical*28 + bestVictimScore*3 - selfPips*38;
    const candidate={tid,score,blockedPips:totalBlockedPips,opp:bestVictim};
...
function App(){
  ...
  const balancedDiceRef=useRef(createBalancedDiceController());
  ...
  function rollDice(){
    return runHumanAction(()=>{
      ...
      const pair=isPVBot?balancedDiceRef.current.roll(String(active.id),players.map(p=>String(p.id))):[1+Math.floor(Math.random()*6),1+Math.floor(Math.random()*6)];
  ...
  function chooseRobberResourceForBot(_thief,victim,_currentPlayers,_currentBoard,_currentBank){
    // Robber resolution: target selection may be strategic, but the actual resource
    // stolen is exactly ONE card of a random resource type held by the victim.
    const held=RES.filter(r=>(victim?.hand?.[r]||0)>0);
    if(!held.length)return null;
    return held[Math.floor(Math.random()*held.length)];
  }
  function stealFromVictim(victim,countAsAction=true,forcedResource=null){
    if(!active||!victim||victim.id===active.id||total(victim.hand)<=0)return false;
    const held=RES.filter(r=>(victim.hand[r]||0)>0);
    if(!held.length)return false;
    const stolen=forcedResource&&held.includes(forcedResource)?forcedResource:held[Math.floor(Math.random()*held.length)];
    if((victim.hand[stolen]||0)<=0)return false;
    const nextPlayers=players.map(p=>p.id===victim.id?{...p,hand:{...p.hand,[stolen]:(p.hand[stolen]||0)-1}}:p.id===active.id?{...p,hand:{...p.hand,[stolen]:(p.hand[stolen]||0)+1}}:p);
    setPlayers(nextPlayers);
    ...
    appendLog(`${active.name} moved the robber and stole 1 ${LABEL[stolen]} card from ${victim.name}.`);
  ...
  useEffect(()=>{
    ...
    const accept=botProb<=0.5;
    window.setTimeout(()=>{if(!drawOffer)return;respondDrawOffer(accept);},0);
  ...
  const modal_body=(quickPanel||devOpen||tradeOffer||discardState||robberVictim||drawOffer||(devChoice&&devChoice.card!=="Road Building"))?<div className="refOverlay quickPanelModal"><div className="refOverlayCard quickPanelCard">{!discardState&&!tradeOffer?.botInitiated&&<button className="refClose" onClick={()=>{setQuickPanel(null);setDevOpen(false);setTradeOffer(null);setRobberVictim(null);cancelActionConfirm()}}>×</button>}<>{quickPanel==="trade"&&<><span className="eyebrow">ACTION DOCK</span><h2>TRADE</h2><div className="tradeModalGrid"><div className="tradeBox"><div className="boxTitle">BANK / PORT</div><div className="tradeRow"><select value={trade.give} onChange={e=>setTrade(t=>({...t,give:e.target.value}))}>{RES.map(r=><option key={r} value={r}>{LABEL[r]}</option>)}</select><b>→</b><select value={trade.get} onChange={e=>setTrade(t=>({...t,get:e.target.value}))}>{RES.map(r=><option key={r} value={r}>{LABEL[r]}</option>)}</select><button onClick={()=>{if(tradeNow())setQuickPanel(null)}}>TRADE</button></div><div className="bankRateBadge">{tradeRate(active,ports,trade.give)}:1 {tradeRate(active,ports,trade.give)===4?"BANK TRADE":"PORT RATE"}</div><small>Default bank exchange is 4 identical resources for 1 resource; your 3:1 or 2:1 port lowers the rate.</small></div><div className="playerTradeBox"><div className="boxTitle">PLAYER TRADE</div><div className="playerTradePartner"><span>Trade with</span><select value={playerTrade.partner} onChange={e=>setPlayerTrade(t=>({...t,partner:Number(e.target.value)}))}>{players.filter(p=>p.id!==active.id).map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></div><div className="bundleTradeGrid"><div className="bundleBox"><b>YOU GIVE</b>{RES.map(r=><label key={r}><span>{ICON[r]} {LABEL[r]}</span><input type="number" min="0" max={active?.hand?.[r]||0} value={playerTrade.giveBundle[r]||0} onChange={e=>setPlayerTrade(t=>({...t,giveBundle:{...t.giveBundle,[r]:e.target.value}}))}/></label>)}<small>Total: {bundleTotal(playerTrade.giveBundle)}</small></div><div className="bundleArrow">⇄</div><div className="bundleBox"><b>YOU RECEIVE</b>{RES.map(r=><label key={r}><span>{ICON[r]} {LABEL[r]}</span><input type="number" min="0" value={playerTrade.getBundle[r]||0} onChange={e=>setPlayerTrade(t=>({...t,getBundle:{...t.getBundle,[r]:e.target.value}}))}/></label>)}<small>Total: {bundleTotal(playerTrade.getBundle)}</small></div></div><p className="tradeRuleHint">Any combination is allowed, but both sides must give something and the same resource cannot appear on both sides.</p><button disabled={players.length===2} onClick={()=>{if(playerTradeNow())setQuickPanel(null)}}>{players.length===2?"DISABLED IN 1v1":"OFFER TRADE"}</button></div></div></>}{quickPanel==="robber"&&<><span className="eyebrow">ACTION DOCK</span><h2>ROBBER</h2><p className="modalHint">The robber is active. Click a highlighted hex on the board to move it and steal a resource.</p><button className="refPrimaryButton" onClick={()=>setQuickPanel(null)}>RETURN TO BOARD</button></>}{devOpen&&<><span className="eyebrow">DEVELOPMENT DECK</span><h2>PLAY A CARD</h2><p className="modalHint">Play an owned development card. Victory Points remain hidden until they can complete the win.</p><div className="refDevModalGrid">{Object.keys(DEV).map(c=><button key={c} disabled={!active?.development[c]||active.bot||winner||drawn||roll===7||robberMode||discardState||(c!=="Victory Point"&&!hasRolled)||(c!=="Victory Point"&&devPlayed)||((c!=="Victory Point")&&((active?.development[c]||0)-(devCardsBought[c]||0)<=0))} onClick={()=>playDev(c)}><span>{DEV_ICON[c]}</span><b>{c}</b><small>Owned: {active?.development[c]||0}</small></button>)}</div></>}{devChoice&&<><span className="eyebrow">CARD RESOLUTION</span><h2>{devChoice.card}</h2><p className="modalHint">{devChoice.card==="Year of Plenty"?"Choose two resources.":devChoice.card==="Monopoly"?"Choose a resource to collect from opponents.":"Choose up to two legal road locations on the board."}</p>{(devChoice.card==="Year of Plenty"||devChoice.card==="Monopoly")&&<div className="refChoiceGrid">{RES.map(r=>{const count=devChoice.resources.filter(x=>x===r).length;const maxAvailable=bank[r]||0;return <button key={r} className={count>0?"selected":""} disabled={devChoice.card==="Year of Plenty"&&(devChoice.resources.length>=2||count>=maxAvailable)} aria-label={devChoice.card==="Year of Plenty"?`${LABEL[r]} available ${maxAvailable}, selected ${count}`:LABEL[r]} onClick={()=>{setDevChoice(d=>{let next;if(d.card==="Monopoly")next={...d,resources:[r]};else if(d.resources.length<2 && d.resources.filter(x=>x===r).length<maxAvailable)next={...d,resources:[...d.resources,r]};else return d;const complete=(next.card==="Monopoly"&&next.resources.length===1)||(next.card==="Year of Plenty"&&next.resources.length===2);if(complete)requestAnimationFrame(()=>applyDevChoice(next.card,next.resources,[],next));return next;})}}>{ICON[r]} {LABEL[r]}{devChoice.card==="Year of Plenty"&&count>0?<small>×{count}</small>:null}</button>})}</div>}{devChoice.card==="Road Building"&&devChoice.roads?.length>0&&<div className="roadBuildResolveRow"><span>{devChoice.roads.length}/2 roads selected</span><button className="refPrimaryButton" onClick={()=>applyDevChoice("Road Building",[],devChoice.roads)}>DONE ROAD BUILDING</button></div>}<div className="autoResolveHint">Year of Plenty and Monopoly resolve automatically when selected. Road Building can use one or two roads.</div></>}{robberVictim&&<><span className="eyebrow">ROBBER VICTIM</span><h2>Choose who to steal from</h2><p className="modalHint">The robber is on the selected hex. Choose one adjacent opponent with resource cards.</p><div className="refChoiceGrid">{robberVictim.victims.map(id=>{const v=players.find(p=>p.id===id);return <button key={id} onClick={()=>stealFromVictim(v)}><span>👤</span><b>{v?.name||"Player"}</b><small>{total(v?.hand||empty())} cards</small></button>})}</div></>} {drawOffer&&<><span className="eyebrow">DRAW OFFER</span><h2>{players.find(p=>p.id===drawOffer.from)?.name||"Player"} offered a draw</h2><p className="modalHint">Accept the draw and end the 1v1 match immediately, or decline and continue playing.</p><div className="refOfferButtons"><button className="refPrimaryButton" onClick={()=>respondDrawOffer(true)}>ACCEPT DRAW</button><button className="refDangerButton" onClick={()=>respondDrawOffer(false)}>DECLINE</button></div></>}{tradeOffer&&<><span className="eyebrow">{tradeOffer.botInitiated?"BOT TRADE OFFER":"TRADE OFFER"}</span><h2>{players.find(p=>p.id===tradeOffer.from)?.name} → {players.find(p=>p.id===tradeOffer.to)?.name}</h2><p className="modalHint">Offer: {bundleText(tradeOffer.giveBundle||{[tradeOffer.give]:tradeOffer.giveAmount})} for {bundleText(tradeOffer.getBundle||{[tradeOffer.get]:tradeOffer.getAmount})}.</p><div className="refOfferButtons"><button className="refPrimaryButton" onClick={()=>{respondTradeOffer(true)}}>ACCEPT</button><button className="refDangerButton" onClick={()=>{respondTradeOffer(false)}}>DECLINE</button></div></>}{discardState&&(()=>{const discardPlayer=players.find(p=>p.id===discardState.playerId)||active;return <><span className="eyebrow">ROBBER DISCARDS</span><h2>{discardPlayer?.name||"Player"}: discard {discardState.remaining} cards</h2><p className="modalHint"><b>MANDATORY:</b> You cannot continue until exactly {discardState.remaining} cards are discarded. In 1v1, the safe hand is 9 cards; in standard games it is 7.</p><div className="refChoiceGrid">{RES.map(r=><button key={r} className={discardSelection[r]>0?"selected":""} disabled={(discardPlayer?.hand[r]||0)<=discardSelection[r]} onClick={()=>handleDiscardResourceClick(r)}>{ICON[r]} {LABEL[r]} · {discardPlayer?.hand[r]||0}</button>)}</div><div className="autoResolveHint">Discarding resolves automatically when the required number of cards is selected.</div></>})()}</></div></div>:null;
    ...
    return <div className="app hxGame">
      <header className="hxTop">
        ...
        <nav className="hxNav" aria-label="Game navigation">
          ...
          <button className={`hxTab ${tab==="analysis"?"active":""}`} onClick={()=>setTab("analysis")}><Ico n="gear"/><b>Settings</b></button>
        </nav>
      ...
      <main className="hxMain">
        ...
        <section className="hxCenter">
          ...
          onVertex={v=>{if(winner||drawn||active?.bot||screen!=="playing"||discardState||devChoice?.card||robberMode||roll===null||roll===7)return;
            if(active.settlements?.includes(v)){if(canPay(active.hand,COSTS.city)&&piecesRemaining(active).cities>0)buildCity(v);return;}
            if(legalSettlement(v,players,geo)&&settlementConnected(v,active,geo)&&canPay(active.hand,COSTS.settlement)&&piecesRemaining(active).settlements>0)buildSettlement(v);}}
          ...
          analysisMode={analysisMode} boardAnalysis={boardAnalysis} onRobber={moveRobber} robberActive={robberMode}/></div>
          </div>
        ...
        <aside className="hxRight">
          ...
          <button className="hxPanel hxLink" onClick={()=>setTab("analysis")}><Ico n="chart"/><div><b>Board Analysis</b><small>AI analysis of current board state</small></div><Ico n="chevron"/></button>
