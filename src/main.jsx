...
function App(){
  ...
  function stealFromVictim(victim,countAsAction=true,robberTid=null){
    if(!active||!victim||victim.id===active.id||total(victim.hand)<=0)return false;
    // Resolve the victim from the robber's actual destination, never from the
    // player who initiated the robber action. This guards against stale React
    // state and prevents a robber move from stealing from the acting player.
    const tid=Number.isInteger(robberTid)?robberTid:(Number.isInteger(robberVictim?.tid)?robberVictim.tid:board.findIndex(t=>t.robber));
    ...
    if(!stillEligible||victim.id===active.id)return false;
    const currentVictim=players.find(p=>p.id===victim.id);
    if(!currentVictim||total(currentVictim.hand)<=0)return false;
    const stolen=randomHeldResource(currentVictim);
    if(!stolen||(currentVictim.hand[stolen]||0)<=0)return false;
    const nextPlayers=players.map(p=>p.id===currentVictim.id?{...p,hand:{...p.hand,[stolen]:Math.max(0,(p.hand[stolen]||0)-1)}}:p.id===active.id?{...p,hand:{...p.hand,[stolen]:(p.hand[stolen]||0)+1}}:p);
    setPlayers(nextPlayers);
